...
exports.editMovies = (id, data, cb)=> {
  const sql = `UPDATE "movies" SET "title" = COALESCE(NULLIF($2, ''), "title"), "picure" = COALESCE(NULLIF($3, ''),"picture"), "releaseDate" = COALESCE(NULLIF($4, ''), "releaseDate"), "director" = COALESCE(NULLIF($5, ''), "director"), "duration" = COALESCE(NULLIF($6, ''), "duration"), "synopsis" = COALESCE(NULLIF($7, ''), "synopsis") WHERE id=$1 RETURNING *`;
